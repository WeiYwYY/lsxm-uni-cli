	...
	methods: {
		...
		*/
		appChooseFile({currentWebview,url,name = 'file',header,...formData} = {}) {
			// #ifdef APP-PLUS
			console.log(currentWebview,url,name,header)
				...
				wv.overrideUrlLoading({mode:'reject'},(e)=>{
					...
					id = unescape(id);
					console.log('fileName,id',fileName,id)
					return this.onCommit(
					this.$emit('upload-success',{fileName,data: {id,statusCode: 200}})
					);
				});
			// #endif
		},
					...
					return uni.uploadFile({
						...
						success: (res) => {
							if (res.statusCode==200) {
								...
								//可自行添加后台返回状态验证
								return this.onCommit(this.$emit('upload-success',{fileName,data}));
							}
		...
		*/
		...
		upErr(errText) {
			this.$emit('upload-error',errText);
		},
